...
// // @ts-types="npm:@types/blessed@^0.1.25"
...
function sessionUtilityUi() {
  const terminalSize = tui.canvas.size;
  ...
  const sidebar = new Table({
    ...
    rectangle: {
      column: 0,
      row: 1,
      height: terminalSize.value.rows - 1,
    },
    headers: [
      { title: " Windows " },
    ],
    data: [
      ...Array.from({ length: 20 }, (_, i) => [` Open window ${i + 1} `]),
      [""],
      ...Array.from({ length: 9 }, (_, i) => [` Closed window ${i + 1} `]),
    ],
    charMap: "rounded",
    zIndex: 0,
  });
  new Effect(() => {
    sidebar.rectangle.value.height = terminalSize.value.rows - 1;
  });
}
...
function demoUi() {
  ...
  new TextBox({
    ...
    text: "Example text inside textbox",
  });
  ...
  const viewScrollbar = new Slider({
    ...
    rectangle: new Computed(() => {
      ...
      viewScrollbarRectangle.column = viewRectangle.column +
        viewRectangle.width -
        1;
...
function debugUi() {
  ...
  tui.canvas.on("render", () => {
    ...
    for (const component of tui.components) {
      if (
        component.view.peek() || component.parent !== tui ||
        // @ts-expect-error NOFRAME
        component.NOFRAME ||
    ...
    tui.on("keyPress", ({ ctrl, meta, shift, key }) => {
      if (!ctrl || key !== "r" || meta || shift) return;
