...
//
// Forked from:
// https://deno.land/std@0.206.0/wasi/snapshot_preview1.ts?source=
// https://github.com/denoland/std/blob/b31795879301189559383d3e496c341d3f695201/wasi/snapshot_preview1.ts
...
type FileLike = Deno.FsFile | Pick<Deno.FsFile, 'readSync'> | Pick<Deno.FsFile, 'writeSync'>;

interface FileDescriptor {
  /** File or writable/readable for stdout/stdin */
  file?: FileLike;
  type?: number;
  flags?: number;
  /** Path on real hard drive. */
  path?: string;
  /** Path used by WebAssembly module. */
  vpath?: string;
...
export interface ContextOptions {
  ...
  /** Allow access to the whole filesystem. */
  skipFsCheck?: boolean;
...
export default class Context {
  ...
  constructor(options: ContextOptions = {}) {
    ...
    this.#memory = null!;
    this.#skipFsCheck = Boolean(options.skipFsCheck);

    this.#fds = [
      {
        file: options.stdin ?? Deno.stdin,
        type: FILETYPE_CHARACTER_DEVICE,
        flags: FDFLAGS_APPEND,
      },
      {
        file: options.stdout ?? Deno.stdout,
        type: FILETYPE_CHARACTER_DEVICE,
        flags: FDFLAGS_APPEND,
      },
      {
        file: options.stderr ?? Deno.stderr,
        type: FILETYPE_CHARACTER_DEVICE,
        flags: FDFLAGS_APPEND,
      },
    ];
    ...
    this.exports = {
      ...
      "fd_close": syscall((
        ...
        if (entry.file && 'close' in entry.file) {
          entry.file.close();
        }
      ...
      "fd_datasync": syscall((
        ...
        if (entry.file && 'syncDataSync' in entry.file) {
          entry.file.syncDataSync();
        }
      ...
      "fd_filestat_get": syscall((
        ...
        if (!(entry.file && 'statSync' in entry.file)) {
          return ERRNO_BADF;
        }
        const info = entry.file.statSync();
      ...
      "fd_filestat_set_size": syscall((
        ...
        if (!(entry.file && 'truncateSync' in entry.file)) {
          return ERRNO_BADF;
        }
      ...
      "fd_pread": syscall((
        ...
        if (!(entry.file && 'seekSync' in entry.file && 'readSync' in entry.file)) {
          return ERRNO_BADF;
        }

        const seek = entry.file.seekSync(0, Deno.SeekMode.Current);
        const memoryView = new DataView(this.#memory.buffer);
        ...
        for (let i = 0; i < iovsLength; i++) {
          ...
          nread += entry.file.readSync(data) as number;
        }

        entry.file.seekSync(seek, Deno.SeekMode.Start);
      ...
      "fd_pwrite": syscall((
        ...
        if (!(entry.file && 'seekSync' in entry.file && 'writeSync' in entry.file)) {
          return ERRNO_BADF;
        }
        ...
        for (let i = 0; i < iovsLength; i++) {
          ...
          nwritten += entry.file.writeSync(data) as number;
        }
      ...
      "fd_read": syscall((
        ...
        if (!(entry.file && 'readSync' in entry.file)) {
          return ERRNO_BADF;
        }
      ...
      "fd_renumber": syscall((
        ...
        {
          const toFd = this.#fds[to].file;
          if (toFd && 'close' in toFd) {
            toFd.close();
          }
        }
      ...
      "fd_seek": syscall((
        ...
        if (!(entry.file && 'seekSync' in entry.file)) {
          return ERRNO_BADF;
        }

        const memoryView = new DataView(this.#memory.buffer);

        const newOffset = entry.file.seekSync(BigInt(offset), whence);
      ...
      "fd_sync": syscall((
        ...
        if (!(entry.file && 'syncSync' in entry.file)) {
          return ERRNO_BADF;
        }
      ...
      "fd_tell": syscall((
        ...
        const offset = entry.file.seekSync(0, Deno.SeekMode.Current);
      ...
      "fd_write": syscall((
        ...
        if (!(entry.file && 'writeSync' in entry.file)) {
          return ERRNO_BADF;
        }
      ...
      "path_open": syscall((
        ...
        if (!this.#skipFsCheck && relative(entry.path, resolvedPath).startsWith("..")) {
        ...
        if (
          ...
          try {
            ...
            if (!this.#skipFsCheck && relative(entry.path, path).startsWith("..")) {
              return ERRNO_NOTCAPABLE;
            }
          } catch (_err) {
            path = resolvedPath;
