export const CODE = `
export function messageToWorker(message) {
    return message;
}
export function messageFromWorker(message) {
    return message;
}
export async function runWasmCommand(options) {
        ...
        stderr: {
            writeSync(buf) {
                if (stderrLen + buf.length > stderr.length) {
                    const larger = new Uint8Array(Math.max(stderr.length * 2, stderrLen + buf.length));
        ...
        stdout: {
            writeSync(buf) {
                if (stdoutLen + buf.length > stdout.length) {
                    const larger = new Uint8Array(Math.max(stdout.length * 2, stdoutLen + buf.length));
        ...
        this.exports = {
            ...
            "fd_pread": syscall((fd, iovsOffset, iovsLength, offset, nreadOffset) => {
                ...
                for (let i = 0; i < iovsLength; i++) {
                    ...
                    nread += entry.file.readSync(data);
                }
            ...
            "fd_pwrite": syscall((fd, iovsOffset, iovsLength, offset, nwrittenOffset) => {
                ...
                for (let i = 0; i < iovsLength; i++) {
                    ...
                    iovsOffset += 4;
                    const data = new Uint8Array(this.#memory.buffer, dataOffset, dataLength);
                    nwritten += entry.file.writeSync(data);
                }
                entry.file.seekSync(seek, Deno.SeekMode.Start);
